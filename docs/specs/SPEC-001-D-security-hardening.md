...
当前项目状态为：SPEC-001 MVP Functional Complete，但 Closure 被 Blocking Issues 阻塞。

本 SPEC 仅处理以下 Blocking Issues：
...
- disabled user 可能继续访问业务数据；
- 降权后的用户可能继续以旧角色执行操作；
- 服务端授权结果与数据库中的权威用户状态不一致。

### AUTH-002：Inspection detail 授权入口不一致

检测台账列表已接入 SPEC-001-A 冻结的 Data Scope / Resource Permission 入口。

但 inspection detail API 使用独立 owner/admin 判断，未统一经过 `inspection_ledger` 资源授权。

风险：

- 不同检测读取入口权限行为不一致；
- `engineer` 等角色可能通过 detail endpoint 读取按冻结矩阵应返回 403 的检测明细；
- list/detail/export/analysis 无法形成统一授权闭环。
...
- 检查并统一 inspection 所有读取入口的授权行为；
- inspection list、detail、export、analysis 必须符合 SPEC-001-A Data Scope Authorization 冻结规则；
- 补充覆盖六身份的授权回归测试。
...
本 SPEC 第一阶段采用：Server-side authoritative user check。
...
- disabled user 不得继续访问受保护 API；
- role 变更后服务端授权不得继续依赖旧 token role；
- 不引入复杂会话管理系统。

### Server Authorization Strategy
...
### Resource Permission Unified Entry

inspection 读取入口必须统一经过冻结的资源权限入口。

要求：

- list 使用 `inspection_ledger` 授权；
- detail 使用与 list 一致的资源授权；
- export 保持导出权限规则；
- analysis 保持参数分析权限规则；
- inspection detail 不允许使用 owner/admin 作为独立授权入口；
...
- 所有 inspection endpoint 必须共享同一授权入口。
...
- 停用用户旧会话失权；
- 角色变更后旧权限不再生效；
- inspection detail 不再绕过 `inspection_ledger` 资源授权；
...
1. 完成代码修改；
2. 完成六身份权限测试；
3. 完成 inspection list / detail / export / analysis 授权回归测试；
4. GitHub Actions 验证通过；
5. Railway 或目标运行环境完成必要运行时验证；
6. 重新执行 SPEC-001 Review 中 AUTH-001 / AUTH-002 检查；
7. 在 Review / Closure 文档中记录关闭证据。

AUTH-001 / AUTH-002 关闭后，SPEC-001 Closure 仍需等待其他 Blocking Issues 关闭后再重新执行。
