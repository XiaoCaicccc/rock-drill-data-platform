...
for (const [code, { num, opt }] of Object.entries(CATEGORY_PARAMS)) {
  const total = num.length + opt.length
  if (total !== 40) throw new Error(`类别 ${code} 参数总数 ${total} ≠ 40`)
}
...
/** 生成数值型检测值，约 85% 在标准区间内 */
function genNumValue(smin: number, smax: number, omin: number, omax: number) {
  const range = smax - smin
  let value: number
  if (Math.random() < 0.85) {
    value = smin + Math.random() * range
  } else {
    const overflow = range * 0.1
    value =
      Math.random() < 0.5
        ? smin - Math.random() * overflow
        : smax + Math.random() * overflow
  }
  const is_qualified = value >= smin && value <= smax
  const is_optimal = value >= omin && value <= omax
  return { value: Math.round(value * 10000) / 10000, is_qualified, is_optimal }
}
...
/** 选项型"合格"值列表（用于 is_qualified 判定） */
const QUALIFIED_OPTIONS = new Set([
  '良好', '均匀细小', '正常', '合格', '无缺陷', '无泄漏', '达标',
  '符合', '完整', '匹配', '一致', '灵活', '6H', '微漏',
])

function isOptQualified(text: string): boolean {
  return QUALIFIED_OPTIONS.has(text)
}
...
export async function POST() {
  ...
  try {
    ...
    const categories = await Promise.all(
      categoryDefs.map((c) =>
        db.part_category.create({
          data: { name: c.name, code: c.code, description: c.description, standard_param_count: 40 },
        }),
      ),
    )
    ...
    // ── 3. 设备档案 ──
    const equipmentDefs = [
      { machine_no: 'YT28-2025-001', model: 'YT28', manufacturer: '天水凿岩机厂', production_date: new Date('2024-06-15'), status: '在用', current_location: '贵州某隧道项目', total_working_hours: 2350 },
      { machine_no: 'HY200-2025-002', model: 'HY200', manufacturer: '阿特拉斯·科普柯', production_date: new Date('2023-11-20'), status: '维修中', current_location: '维修车间', total_working_hours: 4180 },
      { machine_no: 'DZ900-2025-003', model: 'DZ900', manufacturer: '山特维克', production_date: new Date('2022-03-10'), status: '库存', current_location: '配件仓库', total_working_hours: 8560 },
    ]
    ...
    // ── 4. 零件档案（12 件，每类别 2 件） ──
    // 设备1(ZT×2 + HS×2)  设备2(QG×2 + FZ×2)  设备3(ZC×2 + MF×2)
    const partDefs = [
      // 设备 1: 钻头
      { code: 'ZT-001', name: '十字钻头-A1', category_id: categories[0].id, equipment_id: equipmentList[0].id, specification: 'φ48mm', material: '42CrMo+YG15C', supplier: '三一智装', working_hours: 1850, status: '在用' },
      { code: 'ZT-002', name: '球齿钻头-A2', category_id: categories[0].id, equipment_id: equipmentList[0].id, specification: 'φ48mm', material: '40CrNiMo+YG8C', supplier: '徐工供应', working_hours: 1200, status: '在用' },
      // 设备 1: 活塞
      { code: 'HS-001', name: '冲击活塞-A1', category_id: categories[1].id, equipment_id: equipmentList[0].id, specification: 'φ120×280mm', material: '20CrMnTi渗碳', supplier: '自有生产', working_hours: 1650, status: '在用' },
      { code: 'HS-002', name: '配气活塞-A2', category_id: categories[1].id, equipment_id: equipmentList[0].id, specification: 'φ100×220mm', material: '40Cr表面淬火', supplier: '自有生产', working_hours: 980, status: '在用' },
      // 设备 2: 气缸
      { code: 'QG-001', name: '前缸体-B1', category_id: categories[2].id, equipment_id: equipmentList[1].id, specification: 'φ150×400mm', material: 'ZG270-500', supplier: '自有生产', working_hours: 3200, status: '在用' },
      { code: 'QG-002', name: '后缸体-B2', category_id: categories[2].id, equipment_id: equipmentList[1].id, specification: 'φ150×400mm', material: 'ZG270-500', supplier: '自有生产', working_hours: 3200, status: '待检' },
      // 设备 2: 阀组
      { code: 'FZ-001', name: '配气阀组-B1', category_id: categories[3].id, equipment_id: equipmentList[1].id, specification: 'φ60×80mm', material: '40Cr+GCr15', supplier: '中联供应', working_hours: 2800, status: '在用' },
      { code: 'FZ-002', name: '换向阀-B2', category_id: categories[3].id, equipment_id: equipmentList[1].id, specification: 'φ55×70mm', material: '20CrMnTi', supplier: '山推供应', working_hours: 1500, status: '已更换' },
      // 设备 3: 轴承
      { code: 'ZC-001', name: '主轴承-C1', category_id: categories[4].id, equipment_id: equipmentList[2].id, specification: '60×90×22mm', material: 'GCr15', supplier: '进口SKF', working_hours: 7500, status: '在用' },
      { code: 'ZC-002', name: '前端轴承-C2', category_id: categories[4].id, equipment_id: equipmentList[2].id, specification: '80×130×28mm', material: 'GCr15SiMn', supplier: '进口NSK', working_hours: 6200, status: '在用' },
      // 设备 3: 密封件
      { code: 'MF-001', name: '活塞密封组-C1', category_id: categories[5].id, equipment_id: equipmentList[2].id, specification: 'φ120×φ150mm', material: 'NBR+PU', supplier: '鼎基密封', working_hours: 5400, status: '报废' },
      { code: 'MF-002', name: '缸体密封组-C2', category_id: categories[5].id, equipment_id: equipmentList[2].id, specification: 'φ180×φ210mm', material: 'FKM', supplier: '鼎基密封', working_hours: 3800, status: '在用' },
    ]
    const parts = await Promise.all(
      partDefs.map((p) =>
        db.part.create({ data: { ...p, install_date: new Date('2024-07-01') } }),
      ),
    )

    // ── 5. 参数模板 + 参数项（每类别 40 项） ──
    const catCodeMap = Object.fromEntries(categories.map((c) => [c.id, c.code]))
    ...
    for (const cat of categories) {
      ...
      if (!defs) throw new Error(`未找到类别 ${code} 的参数定义`)
      ...
      const created = await db.parameter_item.createMany({ data: itemData })
      // 获取刚创建的参数项 ID（通过模板查询）
      const items = await db.parameter_item.findMany({
    ...
    for (const p of parts) {
      ...
      if (!partsByEquip[p.equipment_id]) partsByEquip[p.equipment_id] = []
      partsByEquip[p.equipment_id].push(p)
    }

    // 建立零件 → 参数项 ID 映射（通过零件的类别）
    const paramMapByPart: Record<string, string[]> = {}
    for (const p of parts) {
      paramMapByPart[p.id] = paramItemIdsByCategory[p.category_id]
    }

    // 检测计划：设备1(在用) 18次 + 设备2(维修中) 8次 + 设备3(库存) 4次 = 30次
    // 每次检测该设备的所有零件（4件）× 40参数 = 160条明细
    // 30 × 160 = 4800 条
    const inspectionSchedule: Array<{ equipment_id: string; date: Date }> = []

    // 设备1：2025-04 ~ 2025-05，约每3-4天一次，18次
    for (let i = 0; i < 18; i++) {
      const dayOffset = Math.round((i / 17) * 59) // 0~59天
      inspectionSchedule.push({
        equipment_id: equipmentList[0].id,
        date: new Date(2025, 3, 1 + dayOffset),
      })
    }
    // 设备2：8次
    for (let i = 0; i < 8; i++) {
      const dayOffset = Math.round((i / 7) * 59)
      inspectionSchedule.push({
        equipment_id: equipmentList[1].id,
        date: new Date(2025, 3, 1 + dayOffset),
      })
    }
    // 设备3：4次
    for (let i = 0; i < 4; i++) {
      const dayOffset = Math.round((i / 3) * 59)
      inspectionSchedule.push({
        equipment_id: equipmentList[2].id,
        date: new Date(2025, 3, 1 + dayOffset),
      })
    }
    ...
    // 预加载所有参数项（含区间信息）
    const allParamItems = await db.parameter_item.findMany({
      select: { id: true, standard_min: true, standard_max: true, optimal_min: true, optimal_max: true, data_type: true, options: true },
    })
    const paramMap: Record<string, (typeof allParamItems)[number]> = Object.fromEntries(
      allParamItems.map((pi) => [pi.id, pi]),
    )

    // 预加载各类别的参数定义（用于生成值）
    const catParamsByCode: Record<string, { num: NumP[]; opt: OptP[] }> = CATEGORY_PARAMS

    for (const schedule of inspectionSchedule) {
      const eqParts = partsByEquip[schedule.equipment_id] ?? []

      const recordNo = `JY-2025-${String(recordIdx).padStart(4, '0')}`
      const month = schedule.date.getMonth() + 1
      const week = Math.ceil(schedule.date.getDate() / 7)
      const batchNo = `PC-2025${String(month).padStart(2, '0')}-${String(week).padStart(2, '0')}`
      ...
      for (const part of eqParts) {
        const pItemIds = paramMapByPart[part.id] ?? []
        const catCode = catCodeMap[part.category_id]
        const catDefs = catParamsByCode[catCode]

        // 构建该类别参数的编码→定义映射
        const numDefMap = new Map<string, NumP>()
        const optDefMap = new Map<string, OptP>()
        if (catDefs) {
          for (const d of catDefs.num) numDefMap.set(d[1], d)
          for (const d of catDefs.opt) optDefMap.set(d[1], d)
        }

        for (let pi = 0; pi < pItemIds.length; pi++) {
          const pItem = paramMap[pItemIds[pi]]
          if (!pItem) continue
          total++

          if (pItem.data_type === 'option' && pItem.options) {
            const opts = JSON.parse(pItem.options) as string[]
            const { text, is_qualified, is_optimal } = genOptValue(opts)
            if (is_qualified) qualified++
            batchData.push({
              record_id: record.id,
              part_id: part.id,
              param_item_id: pItemIds[pi],
              value_number: null,
              value_text: text,
              is_qualified,
              is_optimal,
            ...
            const smin = pItem.standard_min ?? 0
            const smax = pItem.standard_max ?? 0
            const omin = pItem.optimal_min ?? 0
            const omax = pItem.optimal_max ?? 0
            const { value, is_qualified, is_optimal } = genNumValue(smin, smax, omin, omax)
            if (is_qualified) qualified++
            batchData.push({
              record_id: record.id,
              part_id: part.id,
      ...
      for (let i = 0; i < batchData.length; i += 500) {
        await db.inspection_data_item.createMany({ data: batchData.slice(i, i + 500) })
        totalDataItems += Math.min(500, batchData.length - i)
      }
      // 修正最后一批计数
      if (batchData.length > 500) {
        totalDataItems = batchData.length * recordIdx // 简化：用累积方式
      }

      // 更新整体结果
      const passRate = total > 0 ? qualified / total : 0
      const overall_result = passRate >= 0.95 ? '合格' : passRate >= 0.85 ? '待复检' : '不合格'
      await db.inspection_record.update({
        where: { id: record.id },
        data: { overall_result },
      })
    ...
    // 精确统计
    totalDataItems = await db.inspection_data_item.count()

    // ── 7. 分析报告 ──
    await db.analysis_report.createMany({
      data: [
        {
          report_no: 'BG-2025-04-YD001', title: '2025年4月凿岩机零部件质量分析月报',
          type: '月度分析', period: '2025-04', status: '已发布', author: '张伟',
          summary: '4月份共完成20次设备检测，覆盖12个零部件，检测参数3200项，整体合格率91.2%。',
          conclusion: '1. 加强气缸内壁粗糙度波动管控；2. 钻头钎焊质量稳定，继续保持。',
        },
        {
          report_no: 'BG-2025-05-YD001', title: '2025年5月凿岩机零部件质量分析月报',
          type: '月度分析', period: '2025-05', status: '已发布', author: '张伟',
          summary: '5月份共完成10次设备检测，整体合格率92.8%，较4月提升1.6个百分点。',
          conclusion: '5月质量指标持续向好。建议关注维修设备HY200-002各零件磨损数据。',
        },
        {
          report_no: 'BG-2025-Q2-JD001', title: '2025年Q2季度凿岩机零部件全生命周期分析报告',
          type: '季度分析', period: '2025-Q2', status: '草稿', author: '李明',
          summary: '第二季度整体质量稳中有升，零部件寿命指标达到设计标准的1.1倍。',
          conclusion: '建议下季度重点关注高磨损件寿命优化及新型密封材料应用对比。',
        },
        {
          report_no: 'BG-2025-ZX-001', title: '钻头类零部件专项寿命分析报告',
          type: '专项分析', period: '2025-H1', status: '已发布', author: '王芳',
          summary: '对十字钻头ZT-001和球齿钻头ZT-002进行连续跟踪，球齿钻头单位磨损率低于十字钻头32%。',
          conclusion: '球齿钻头性价比最优，建议高磨损工况优先选用。',
        },
      ],
    })
    ...
    const meeting1 = await db.meeting.create({
      data: {
        title: '4月份质量分析例会', meeting_date: new Date('2025, 4, 6, 9, 0'),
        location: '三楼会议室A', organizer: '张伟',
        participants: '张伟,李明,王芳,刘强,陈静',
        minutes_content: '1. 通报4月检测数据总体情况；2. 讨论气缸内壁粗糙度波动问题；3. 安排5月检测计划。',
        status: '已完成',
      },
    })
    const meeting2 = await db.meeting.create({
      data: {
        title: '5月份质量分析例会', meeting_date: new Date('2025, 5, 6, 9, 0'),
        location: '三楼会议室A', organizer: '张伟',
        participants: '张伟,李明,王芳,刘强,陈静',
        minutes_content: '1. 5月合格率提升至92.8%；2. 维修设备HY200-002送检结果分析；3. 部署球齿钻头寿命验证试验。',
        status: '已完成',
      },
    })
    await db.meeting_resolution.createMany({
      data: [
        { meeting_id: meeting1.id, content: '加强前缸体供应商内壁精加工工艺管控', responsible_person: '王芳', due_date: new Date('2025-04-20'), status: '已完成' },
        { meeting_id: meeting1.id, content: '编制钻头钎焊质量专项分析报告', responsible_person: '陈静', due_date: new Date('2025-04-15'), status: '已完成' },
        { meeting_id: meeting2.id, content: '推进球齿钻头与十字钻头对比寿命验证试验', responsible_person: '刘强', due_date: new Date('2025-06-30'), status: '待执行' },
      ],
    })
    ...
    const attData: Array<{ date: Date; member_name: string; status: string; remark: string | null }> = []
    for (let month = 4; month <= 5; month++) {
      const daysInMonth = month === 4 ? 30 : 31
      for (let day = 1; day <= daysInMonth; day++) {
        if (new Date(2025, month - 1, day).getDay() === 0) continue // 跳过周日
        if (new Date(2025, month - 1, day).getDay() === 6) continue // 跳过周六
        const date = new Date(2025, month - 1, day)
        for (const m of members) {
          const r = Math.random()
          let status: string, remark: string | null = null
          if (r < 0.88) { status = '出勤' }
          else if (r < 0.93) { status = '请假'; remark = '事假' }
          else if (r < 0.97) { status = '迟到'; remark = '迟到10分钟' }
          else { status = '出差'; remark = '供应商审核' }
          attData.push({ date, member_name: m, status, remark })
        }
      }
    }
    for (let i = 0; i < attData.length; i += 200) {
      await db.attendance_record.createMany({ data: attData.slice(i, i + 200) })
    }
    ...
    const counts = {
      ...
      inspection_data_item: totalDataItems,
