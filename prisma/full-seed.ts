...
async function main() {
  ...
  try {
    ...
    await Promise.all([
      ...
      db.document.create({
        data: { title: '2025年5月质量分析月报', category: '报告', relatedReportId: null, archived: true, remark: '正式发布版' },
      }),
      db.document.create({
        data: { title: '钻头类专项寿命分析报告', category: '报告', relatedReportId: null, archived: true, remark: '已发布' },
      }),
      db.document.create({
        data: { title: '4月份质量分析例会纪要', category: '纪要', relatedReportId: null, archived: true, remark: '已归档' },
      }),
      db.document.create({
        data: { title: '5月份质量分析例会纪要', category: '纪要', relatedReportId: null, archived: true, remark: '已归档' },
      }),
      db.document.create({
        data: { title: '新检测设备采购评审会纪要', category: '纪要', relatedReportId: null, archived: false, remark: '待归档' },
      }),
      db.document.create({
        data: { title: '凿岩机零部件检测规范 V2.1', category: '标准', filePath: '/docs/standards/inspection-v2.1.pdf', archived: true, remark: '现行有效版本' },
      }),
      db.document.create({
        data: { title: 'GB/T 6379 测量方法与结果', category: '标准', filePath: '/docs/standards/GBT-6379.pdf', archived: true, remark: '参考标准' },
      }),
      db.document.create({
        data: { title: '数据分析师岗位作业指导书', category: '制度', filePath: '/docs/rules/job-guide.pdf', archived: true, remark: '部门内部文件' },
      }),
      db.document.create({
